...
function render(){
  ...
  root.innerHTML='<div class="timeline">'+list.map(e=>'<article class="event-item"><span class="event-dot" aria-hidden="true"></span><div class="event-card card-hover"><button class="event-main" data-id="'+esc(e.id)+'" aria-expanded="false" onclick="toggleEvent(this.dataset.id,this)"><div class="event-date">'+esc(e.release_date||T("events_unknown_date"))+'</div><h2 class="event-name">'+esc(EVENT_ICON+' '+e.name)+'</h2><p class="event-summary">'+esc(e.summary||T("no_data"))+'</p><div class="event-meta"><span class="stars" aria-label="'+esc(T("importance_label"))+' '+esc(e.importance||0)+'">'+stars(e.importance)+'</span><span class="expand-hint">'+T("events_expand")+'</span></div></button><div class="event-detail" id="detail-'+esc(e.id)+'"></div></div></article>').join("")+'</div>';
}
